...
public class analysisClass {
	...
	/* Momentum array */
	ArrayList<Double> Parray; // Index of lower-bound momenta (a part from latest, that is last bin
	...
	ArrayList<H2F> h2_ThetaPhiQMREF_vsP;
	...
	public void run() {
		...
		while (reader.hasEvent() == true && nevent < nEvents) {
			...
			for (TriggerTrack track : tracks) {
				...
				if (imom >= 0) h2_ThetaPhiAllGEN_vsP.get(imom).fill(phi, theta);
				nTracks++;
				...
				if (trigger.getResult() == 0)
					...
					if (dMin < this.minDistance) {
						...
						if (imom >= 0) h2_ThetaPhiAllREC_vsP.get(imom).fill(phi, theta);
					}
			...
			/*
			 * Need to repeat the SAME operation using ALL crosses - that are the objects
			 * actually seen by the trigger!
			 */
	...
	private void setupGeo() {
		...
		/*
		 * Define the EC-plane, in the sector system (i.e. the plane of sector n.1)
		 */
	...
	private void setupHistograms() {
		...
		// h2_ThetaPhiAllEFF = new H2F("h2_ThetaPhiAllEFF", "h2_ThetaPhiAllEFF", 100,
		// -180., 180., 100, 0, 45.);
	...
	private void showHistograms() {
		...
		TCanvas trk2 = new TCanvas("allP", 1600, 1000);
		trk2.divide(3, 2);
		trk2.cd(0);
		...
		for (int ii = 0; ii < Parray.size() - 1; ii++) {
			...
			TCanvasArray.get(ii).divide(3, 2);
			...
			TCanvasArray.get(ii).draw(h2_ThetaPhiAllGEN_vsP.get(ii), "colz");

			TCanvasArray.get(ii).cd(4);
			TCanvasArray.get(ii).draw(h2_ThetaPhiAllREC_vsP.get(ii), "colz");

			TCanvasArray.get(ii).cd(5);
			TCanvasArray.get(ii).draw(h2_ThetaPhiAllEFF_vsP.get(ii), "colz");
	...
	public int getMomentumIndex(double P) {
		int index = -1;
		for (int ii = 0; ii < Parray.size() - 1; ii++) {
			if ((P > Parray.get(ii)) && (P < Parray.get(ii + 1))) index = ii;
		}
