...
public class AnalysisClass {
	...
	private static double thetaAngleDC = 25.;
	private static int nSectors = 6;

	private double l0EC; /*
							 * The distance between the target and the EC plane along the 25 deg line
							 */
	private double bEC, hEC; /* The EC base and height */
	private double dEC; /*
						 * The distance between the triangle vertex and the 25-deg line intersection
						 * point
						 */
	...
	int nevent = -1;
	int nR3Crosses = 0;
	int nR3CrossesMatchedToTracks = 0;
	int nTracksWithR3Cross = 0;
	int nTracksQPWithR3Cross = 0;
	int nTracksQMWithR3Cross = 0;
	...
	private int makeGeneratedParticles(DataBank genParticlesDB, List<Particle> genParticles) {
		...
		for (int loop = 0; loop < nrows; loop++) {
			Particle genParticle = new Particle(genParticlesDB.getInt("pid", loop), genParticlesDB.getFloat("px", loop), genParticlesDB.getFloat("py", loop),
					genParticlesDB.getFloat("pz", loop), genParticlesDB.getFloat("vx", loop), genParticlesDB.getFloat("vy", loop),
	...
	private void makeTriggerTracks(DataBank tracksHBDataBank, DataBank crossesHBDataBank, List<TriggerTrack> tracks, List<Cross> crosses) {
		...
		for (int itrack = 0; itrack < nTracks; itrack++) {
			...
			p0.rotateY(Math.toRadians(AnalysisClass.thetaAngleDC));
			u.rotateY(Math.toRadians(AnalysisClass.thetaAngleDC));

			ep0.rotateY(Math.toRadians(AnalysisClass.thetaAngleDC));
			eu.rotateY(Math.toRadians(AnalysisClass.thetaAngleDC));
		...
		for (int loop = 0; loop < nCrosses; loop++) {
			...
			p0.rotateY(Math.toRadians(thetaAngleDC));
			u.rotateY(Math.toRadians(thetaAngleDC));

			ep0.rotateY(Math.toRadians(thetaAngleDC));
			eu.rotateY(Math.toRadians(thetaAngleDC));
			...
			if (sector==1) {
				double y=p0.y();
				double x=p0.x()*Math.sin(Math.toRadians(thetaAngleDC))-p0.z()*Math.cos(Math.toRadians(thetaAngleDC));
	...
	private void matchTracksToClusters(List<TriggerTrack> tracks, List<ECCluster> clusters[]) {
		double dMatching;
		ArrayList<Double> theMatchingResults = new ArrayList<Double>();
		...
		for (TriggerTrack track : tracks) {
			...
			/*
			 * Do the matching between this track and the clusters in this sector
			 */
			dMatching = this.ECMatcher.matchCrossToClusters(track, clusters[track.get_Sector() - 1]);
			theMatchingResults.add(new Double(dMatching));
			...
			if (dMatching < 0)
				...
				if (this.ECMatcher.distanceIsSmallerThanMin(dMatching)) {
	...
	private void matchCrossesToClusters(List<Cross> crosses, List<ECCluster> clusters[]) {
		...
		for (Cross cross : crosses) {
			...
			dMin = this.ECMatcher.matchCrossToClusters(cross, clusters[cross.get_Sector() - 1]);
			if (dMin < 0)
		...
		/*
		 * Now, matchingDistances contains, for each cross in this event that has been
		 * matched, the corresponding distance
		 */
		...
		/*
		 * Above histograms were made considering ALL crosses. I repeat, considering
		 * only crosses that are matched to a track
		 */

		if (nR3CrossesMatchedToTracks == 3) {
			for (int ibin = 0; ibin < h1_vsDistance1REC.getxAxis().getNBins(); ibin++) {
				thisD = h1_vsDistance1REC.getxAxis().getBinCenter(ibin);
				nMatchings = 0;
				...
				if ((thisD > 49.6) && (thisD < 50.6)) {
					System.out.println(nevent + " : " + nR3CrossesMatchedToTracks + " " + nMatchings + " " + thisD);
				}
	...
	private int readFTOFHits(DataBank hitsFTOFDataBank, List<SimpleTOFHit>[] hitsFTOF2) {
...
//			System.out.println(sector+" "+layer+" "+component+" "+energyL+" "+energyR+" "+timeL+" "+timeR);
			...
			case 2:
				break;
			case 3: // panel2
				if ((energyL > this.minE_TOF2) && (energyR > this.minE_TOF2)) {
					hitsFTOF2[sector - 1].add(hit);
					h2_FTOF2EnergyLR.fill(energyL, energyR);
					break;
				}
			}
		}
	...
	/*
	 * layer is the ID of the TOF system 1 -> panel 1A 2 -> panel 1B 3 -> panel 2
	 */
	private void matchTracksToFTOFHits(int layer, List<TriggerTrack> tracks, List<SimpleTOFHit> hits[]) {
		switch (layer) {
		case 1:
		case 2:
			return; // not yiet implemented
		case 3:
			this.matchTracksToFTOF2Hits(tracks, hits);
			return;
		}
	}

	private void matchTracksToFTOF2Hits(List<TriggerTrack> tracks, List<SimpleTOFHit> hits[]) {
		for (TriggerTrack track : tracks) {
			this.FTOF2Matcher.matchCrossToFTOFHits(track, hits[track.get_Sector() - 1]);
		}
	}
	...
	public void setup(String[] args) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			} else if (splited[0].contains("minE_TOF2")) {
				this.minE_TOF2 = Double.parseDouble(splited[1]);
			}
		}
		this.setupMomentum();
	...
	private void setupHistograms() {
		...
		for (int isector = 1; isector <= 6; isector++) {
			h1_minCrossClusterDistance
					.add(new H1F("h1_minCrossClusterDistance_" + isector, "minCrossClusterDistance_" + isector + ";distance - cm", 100, 0, 500));
		...
		h1_vsDistanceAllREC = new H1F("h1_vsDistanceAllREC", "h1_vsDistanceAllREC", 400, 0., 400.);allH1F.add(h1_vsDistanceAllREC);
		h1_vsDistanceQPREC = new H1F("h1_vsDistanceQPREC", "h1_vsDistanceQPREC", 400, 0., 400.);allH1F.add(h1_vsDistanceQPREC);
		h1_vsDistanceQMREC = new H1F("h1_vsDistanceQMREC", "h1_vsDistanceQMREC", 400, 0., 400.);allH1F.add(h1_vsDistanceQMREC);

		h1_vsDistance1REC = new H1F("h1_vsDistance1REC", "h1_vsDistance3REC", 400, 0., 400.);allH1F.add(h1_vsDistance1REC);
		h1_vsDistance2REC = new H1F("h1_vsDistance2REC", "h1_vsDistance2REC", 400, 0., 400.);allH1F.add(h1_vsDistance2REC);
		h1_vsDistance3REC = new H1F("h1_vsDistance3REC", "h1_vsDistance3REC", 400, 0., 400.);allH1F.add(h1_vsDistance3REC);

		h1_vsDistance1MatchedCrossesREC = new H1F("h1_vsDistance1MatchedCrossesREC", "h1_vsDistance1MatchedCrossesREC", 400, 0., 400.);
		allH1F.add(h1_vsDistance1MatchedCrossesREC);
		h1_vsDistance2MatchedCrossesREC = new H1F("h1_vsDistance2MatchedCrossesREC", "h1_vsDistance2MatchedCrossesREC", 400, 0., 400.);
		allH1F.add(h1_vsDistance2MatchedCrossesREC);
		h1_vsDistance3MatchedCrossesREC = new H1F("h1_vsDistance3MatchedCrossesREC", "h1_vsDistance3MatchedCrossesREC", 400, 0., 400.);
		allH1F.add(h1_vsDistance3MatchedCrossesREC);
		...
		for (int ibin = 0; ibin < Parray.size() - 1; ibin++) {
			h1_vsDistanceAllREC_vsP.add(new H1F("h1_vsDistanceAllREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h1_vsDistanceAllEFF_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 400, 0., 400.));
			h1_vsDistanceQPREC_vsP.add(new H1F("h1_vsDistanceQPREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h1_vsDistanceQPEFF_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 400, 0., 400.));
			h1_vsDistanceQMREC_vsP.add(new H1F("h1_vsDistanceQMREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h1_vsDistanceQMEFF_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 400, 0., 400.));
			...
			h2_ThetaPhiAllGEN_vsP.add(new H2F("h2_ThetaPhiAllGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h2_ThetaPhiAllGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
			h2_ThetaPhiAllREC_vsP.add(new H2F("h2_ThetaPhiAllREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h2_ThetaPhiAllREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));

			h2_ThetaPhiQPGEN_vsP.add(new H2F("h2_ThetaPhiQPGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h2_ThetaPhiQPGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
			h2_ThetaPhiQPREC_vsP.add(new H2F("h2_ThetaPhiQPREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h2_ThetaPhiQPREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));

			h2_ThetaPhiQMGEN_vsP.add(new H2F("h2_ThetaPhiQMGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h2_ThetaPhiQMGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
			h2_ThetaPhiQMREC_vsP.add(new H2F("h2_ThetaPhiQMREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1),
					"h2_ThetaPhiQMREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
		}

		/* MIP: ~ 2 MeV / cm, TOF2 thick=5 cm -> MPI=10 MeV */
		h2_FTOF2EnergyLR = new H2F("h2_FTOF2EnergyLR", "h2_FTOFEnergyLR", 100, 0, 30, 100, 0, 30);
	...
	private void processHistograms() {
		...
		h1_vsDistance1MatchedCrossesEFF.divide(1. * nEvents3Rec);
		...
		h1_vsDistance2MatchedCrossesEFF.divide(1. * nEvents3Rec);
	...
	public H1F getHistogram(String name) {
	...
	private void showHistograms() {
		...
		TCanvas ceff = new TCanvas("ceffECALOnly", 1600, 1000);
		ceff.divide(2, 3);
		...
		cTOF2.divide(2, 3);
		cTOF2.cd(0);
		cTOF2.draw(h2_FTOF2EnergyLR, "colz");
	...
	public void run() {
		...
		while (reader.hasEvent() == true && nevent < nEvents) {
			...
			/* Clean variables */
			nR3Crosses = 0;
			nR3CrossesMatchedToTracks = 0;
			nTracksWithR3Cross = 0; /* tracks with a R3 cross! */
			nTracksQPWithR3Cross = 0;
			nTracksQMWithR3Cross = 0;
			...
			/*
			 * if (nReconstructedParticles >= 1) nEvents1Rec++; if (nReconstructedParticles
			 * >= 2) nEvents2Rec++; if (nReconstructedParticles >= 3) nEvents3Rec++;
			 */

			if (tracksHB.rows() >= 1) nEvents1Rec++;
			if (tracksHB.rows() >= 2) nEvents2Rec++;
			if (tracksHB.rows() >= 3) nEvents3Rec++;
			...
			List<SimpleTOFHit>[] hitsFTOF2 = new ArrayList[AnalysisClass.nSectors];
			for (int ii = 0; ii < AnalysisClass.nSectors; ii++) {
				hitsFTOF2[ii] = new ArrayList<SimpleTOFHit>();
			}
			int nHits = readFTOFHits(hitsFTOF, hitsFTOF2);

			/*
			 * Now, I have the list of tracks and the list of clusters - sector by sector.
			 * Need to match them
			 */

			/*
			 * First, do a "direct" analysis: select tracks, get their R3 cross, and check
			 * if there's a matching with ECAL.This analysis is aimed at maximizing the
			 * trigger efficiency for the signal
			 */
			this.matchTracksToClusters(tracks, clusters);
			
			/* Now, do a "direct" analysis for FTOF2 hits*/
			this.matchTracksToFTOFHits(3,tracks,hitsFTOF2);

			/*
			 * Then, do an "inverse" analysis - basically what the trigger does: take all R3
			 * crosses - doesn't matter if matched to a track or not, match them with ECAL.
			 */
