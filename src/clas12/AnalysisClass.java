...
public class AnalysisClass {
    ...
    private static double l0FTOF2 = 605.397; /*
                                              * See my notes on 14/9/2017.
                                              * Extracted from D. Carman
                                              * description of CLAS12 FTOF
                                              */

    private Plane3D planeFTOF2;
    ...
    private void makeTriggerTracks(DataBank tracksHBDataBank, DataBank crossesHBDataBank, List<TriggerTrack> tracks, List<Cross> crosses) {
        ...
        for (int itrack = 0; itrack < nTracks; itrack++) {
            ...
            p0.rotateY(Math.toRadians(thetaAngleEC));
            u.rotateY(Math.toRadians(thetaAngleEC));

            ep0.rotateY(Math.toRadians(thetaAngleEC));
            eu.rotateY(Math.toRadians(thetaAngleEC));
    ...
    private void setupGeoEC() {
         ...
         * n.1)
         */
        Vector3D n = new Vector3D(Math.sin(Math.toRadians(AnalysisClass.thetaAngleEC)), 0., Math.cos(Math.toRadians(AnalysisClass.thetaAngleEC)));
        Point3D p = new Point3D(0, 0, this.l0EC / Math.cos(Math.toRadians(AnalysisClass.thetaAngleEC)));
        planeEC = new Plane3D(p, n);
    }

    private void setupGeoFTOF2() {
        /*
         * Define the FTOF2-plane, in the sector system (i.e. the plane of
         * sector n.1)
         */
    ...
    private void setupHistograms() {
        ...
        for (int ibin = 0; ibin < Parray.size() - 1; ibin++) {
            h1_vsDistanceAllREC_vsP.add(new H1F("h1_vsDistanceAllREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h1_vsDistanceAllEFF_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 400, 0., 400.));
            h1_vsDistanceQPREC_vsP.add(new H1F("h1_vsDistanceQPREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h1_vsDistanceQPEFF_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 400, 0., 400.));
            h1_vsDistanceQMREC_vsP.add(new H1F("h1_vsDistanceQMREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h1_vsDistanceQMEFF_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 400, 0., 400.));

            h2_ThetaPhiAllGEN_vsP.add(new H2F("h2_ThetaPhiAllGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h2_ThetaPhiAllGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
            h2_ThetaPhiAllREC_vsP.add(new H2F("h2_ThetaPhiAllREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h2_ThetaPhiAllREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));

            h2_ThetaPhiQPGEN_vsP.add(new H2F("h2_ThetaPhiQPGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h2_ThetaPhiQPGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
            h2_ThetaPhiQPREC_vsP.add(new H2F("h2_ThetaPhiQPREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h2_ThetaPhiQPREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));

            h2_ThetaPhiQMGEN_vsP.add(new H2F("h2_ThetaPhiQMGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h2_ThetaPhiQMGEN_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
            h2_ThetaPhiQMREC_vsP.add(new H2F("h2_ThetaPhiQMREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), "h2_ThetaPhiQMREC_" + Parray.get(ibin) + "_" + Parray.get(ibin + 1), 100, -180., 180., 100, 0, 45.));
    ...
    public void run() {
        ...
        while (reader.hasEvent() == true && nevent < nEvents) {
            ...
            /*
             * if (nReconstructedParticles >= 1) nEvents1Rec++; if
             * (nReconstructedParticles >= 2) nEvents2Rec++; if
             * (nReconstructedParticles >= 3) nEvents3Rec++;
             */
